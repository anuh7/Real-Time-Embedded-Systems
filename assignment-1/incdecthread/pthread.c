//https://man7.org/linux/man-pages/man3/pthread_setschedparam.3.html
// https://man7.org/linux/man-pages/man3/pthread_join.3.html

#include <pthread.h>    /* It consists of the details and functions associated with threads */
#include <stdlib.h>
#include <stdio.h>
#include <sched.h>      /* scheduling parameters required for implementation of each supported scheduling policy.*/

#define COUNT  1000

/* A structure which will store the thread index parameter */
typedef struct
{
    int threadIdx;
} threadParams_t;


/* POSIX thread declarations and scheduling attributes */ 
pthread_t threads[2];
threadParams_t threadParams[2];


// Unsafe global
int gsum=0;

/*******************************************************************************
* @Function Name: incThread
* @Description: Increments a global variable and prints the thread index, COUNT number of times
* @input param: Thread attributes
* @return: None
*******************************************************************************/
void *incThread(void *threadp)
{
    int i;
    threadParams_t *threadParams = (threadParams_t *)threadp;

    for(i=0; i<COUNT; i++)
    {
        gsum=gsum+i;
        printf("Increment thread idx=%d, gsum=%d\n", threadParams->threadIdx, gsum);
    }
}


/*******************************************************************************
* @Function Name: decThread
* @Description: Decrements a global variable and prints the thread index COUNT number of times
* @input param: Thread attributes
* @return: None
*******************************************************************************/
void *decThread(void *threadp)
{
    int i;
    threadParams_t *threadParams = (threadParams_t *)threadp;

    for(i=0; i<COUNT; i++)
    {
        gsum=gsum-i;
        printf("Decrement thread idx=%d, gsum=%d\n", threadParams->threadIdx, gsum);
    }
}



/*******************************************************************************
* @Function Name: main
* @Description: The function is the main application entry point which creates
* the increment and decrement threads.
* @input param: None
* @author: nodified by rishab shah
* @return: success/failure
*******************************************************************************/
int main (int argc, char *argv[])
{
   int rc;
   int i=0;

   threadParams[i].threadIdx=i;
   pthread_create(&threads[i],   // pointer to thread descriptor
                  (void *)0,     // use default attributes
                  incThread, // thread function entry point
                  (void *)&(threadParams[i]) // parameters to pass in
                 );
   i++;

   threadParams[i].threadIdx=i;
   pthread_create(&threads[i], (void *)0, decThread, (void *)&(threadParams[i]));

   for(i=0; i<2; i++)

/* function waits for the increment and decrement thread to terminate.*/
     pthread_join(threads[i], NULL);

   printf("TEST COMPLETE\n");
}
