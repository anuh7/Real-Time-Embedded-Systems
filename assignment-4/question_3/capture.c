...
static struct timespec time_start, time_stop;
static struct timespec frame_stop, frame_start, frame_dt;
...
static void mainloop(void)
{
    ...
    while (count > 0)
    {
        ...
        delta_t(&frame_stop, &frame_start, &frame_dt);
        wcet = (double)frame_dt.tv_sec +(double)frame_dt.tv_nsec / NSEC;
        ...
        syslog(LOG_INFO, "FPS for each frame processing = %lf, for %d th frame \n", fps, framecnt);
