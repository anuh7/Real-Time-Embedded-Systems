...
static struct timespec service_start, service_end;
static double fservice_start, fservice_end;
static struct timespec frame_stop, frame_start, frame_time;

int read_framecnt=-START_UP_FRAMES;
int process_framecnt=-START_UP_FRAMES;
int save_framecnt=0;

double read_dt=0.0;
double process_dt=0.0;
double save_dt=0.0;
double wcet = 0.0;
double wcet_avg = 0.0;
double wcet_max = 0.0;
...
static void dump_ppm(const void *p, int size, unsigned int tag, struct timespec *time)
{
    ...
    fservice_start = (double)service_start.tv_sec + (double)service_start.tv_nsec / NSEC;
    ...
    fservice_end = (double)service_end.tv_sec + (double)service_end.tv_nsec / NSEC;
    save_dt += (double)(fservice_end- fservice_start);
    syslog(LOG_CRIT, "save_framecnt=%d took %lf and %lf FPS", save_framecnt, (fservice_end-fservice_start), (double)(1) / (fservice_end-fservice_start));
...
static void process_image(const void *p, int size)
{
    ...
    printf("frame %d: ", process_framecnt);
    ...
    else if(fmt.fmt.pix.pixelformat == V4L2_PIX_FMT_YUYV)
    {
        ...
        if(process_framecnt > -1) 
        {
            clock_gettime(CLOCK_MONOTONIC, &service_end);
            fservice_end = (double)service_end.tv_sec + (double)service_end.tv_nsec / NSEC;
            process_dt += (double)(fservice_end- fservice_start);
            syslog(LOG_CRIT, "process_framecnt=%d took %lf and %lf FPS", process_framecnt, (fservice_end-fservice_start), (double)(1) / (fservice_end-fservice_start));
    ...
    switch (io)
    {
        ...
        case IO_METHOD_MMAP:
            ...
            if (read_framecnt == 0)
            {
                 ...
                 fstart = (double)time_start.tv_sec + (double)time_start.tv_nsec / NSEC;
            }
            ...
            clock_gettime(CLOCK_MONOTONIC, &service_end);
            fservice_end = (double)service_end.tv_sec + (double)service_end.tv_nsec / NSEC;
            read_dt += (double)(fservice_end - fservice_start);
            syslog(LOG_CRIT, "read_framecnt=%d took %lf and %lf FPS", read_framecnt, (fservice_end-fservice_start), (double)(1) / (fservice_end-fservice_start));
...
int main(int argc, char **argv)
{
    ...
    printf("Frame acquisition avg dt : %lf or %lf FPS \n", (read_dt/(double)read_framecnt), ((double)read_framecnt/read_dt));
    printf("Frame processing avg dt : %lf or %lf FPS \n", (process_dt/(double)read_framecnt), ((double)read_framecnt/process_dt));
    printf("Frame storage avg dt : %lf or  %lf FPS \n", (save_dt/(double)read_framecnt), ((double)read_framecnt/save_dt));
    printf("Avg WCET=%lf, Max WCET=%lf \n", (double)(wcet_avg)/(double)(read_framecnt), wcet_max);
