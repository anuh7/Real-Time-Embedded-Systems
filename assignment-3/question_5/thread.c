...
struct timespec watchdog_time;
double watchdog_timenow;
...
void *read_state(void *threadp)
{
   ...
   while(!abortTest)
  {
    ...
    pthread_mutex_lock(&lock);
    printf("*********************Read thread*************************");
    clock_gettime(CLOCK_REALTIME, &(read_timestamp));
...
{
 // printf("Watchdog - thread entry \n");
  while(1)
  {
    struct timespec watchdog_timer;
    clock_gettime(CLOCK_REALTIME, &watchdog_timer);
    watchdog_timer.tv_sec += 10;

    int ret = pthread_mutex_timedlock(&lock, &watchdog_timer);
    clock_gettime(CLOCK_REALTIME, &watchdog_time);

    watchdog_timenow = (double)(((watchdog_time.tv_sec)*1000.0) + ((watchdog_time.tv_nsec)/1000000.0));

    if(ret != 0)
    {
      printf("no new data available at %lf \n", watchdog_timenow);
    }
    else
    {
      pthread_mutex_unlock(&lock);
...
int main() 
{
  ...
  rt_param[2].sched_priority=rt_max_prio-1;
    ...
    pthread_create(&threads[2],&rt_sched_attr[2],watchdog,(void *)&(threadParams[2]));
