...
pthread_mutexattr_t rt_safe;        //mutex attributes
...
int main (int argc, char *argv[])
{
   ...
   else if(argc >= 2)       //time for program-intfTime
   {
   ...
   print_scheduler();
   rc=sched_getparam(getpid(), &nrt_param);     //process parameters
   ...
   for(i=0; i<numberOfProcessors; i++)        //numberofprocessors is not set anywhere
   ...
   pthread_attr_setinheritsched(&rt_sched_attr, PTHREAD_EXPLICIT_SCHED);
   pthread_attr_setschedpolicy(&rt_sched_attr, SCHED_RR);       //why is round robin again?
...
void *simpleTask(void *threadp)
{
  ...
  CPU_ZERO(&cpuset);
  pthread_getaffinity_np(thread, sizeof(cpu_set_t), &cpuset);     //???why
