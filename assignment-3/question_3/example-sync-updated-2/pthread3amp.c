...
int main (int argc, char *argv[])
{
   ...
   rc=pthread_mutexattr_getprotocol(&sharedMemSemAttr, &semProtocol);   //////////without setting the protocol?

  if (rc<0 )
  {
    printf("ERROR: pthread_mutexattr_getprotocol or rc is %d\n");
  }
...
//The prioceiling attribute defines the priority ceiling of initialized mutexes, which is the
// minimum priority level at which the critical section guarded by the mutex is executed. In 
//order to avoid priority inversion, the priority ceiling of the mutex shall be set to a priority
// higher than or equal to the highest priority of all the threads that may lock that mutex.
   ...
   // or block until it can successfully lock the mutex, then it shall change the mutex's priority
...
void *startService(void *threadid)
{
   ...
   //rc = pthread_create(&threads[LOW_PRIO_SERVICE], &nrt_sched_attr, criticalSectionTask, (void *)&threadParams[LOW_PRIO_SERVICE]);
                                          //nrt does have scheduling parameter set. min priority is set with rt. nrt not initialsed properly.
                                          //low priority is running simpleTask.
   if (rc)
   {
       printf("ERROR - run with sudo; pthread_create() rc is %d\n", rc);
       perror(NULL);
